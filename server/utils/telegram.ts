...
`.trim();
    ...
    if (response.ok) {
      console.log(`[Telegram Alert Sent] Order #${order.id} push notification sent to Telegram chat (${chatId})`);
      return true;
    } else {
      const errRes = await response.text();
      console.warn(`[Telegram Alert Warning] Telegram API returned error:`, errRes);
      return false;
    }
  } catch (err: any) {
